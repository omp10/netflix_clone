...
export const WatchPage = () => {
  ...
  useEffect(() => {
    const fetchContentData = async () => {
      try {
        const [contentResponse, trailersResponse, similarResponse] = await Promise.all([
          axios.get(`/api/v1/${contentType}/${id}/details`),
          axios.get(`/api/v1/${contentType}/${id}/trailers`),
          axios.get(`/api/v1/${contentType}/${id}/similar`)
        ]);
        setContent(contentResponse.data.content || null); // Set to null if no content
        setTrailers(trailersResponse.data.trailers || []);
        setSimilarContent(similarResponse.data.similar || []);
      } catch (error) {
        if (error.response && error.response.status === 404) {
          setContent(null); // Set to null on 404
        }
        setTrailers([]);
        setSimilarContent([]);
      } finally {
